...
#define DRAW_LINE(row1, col1, row2, col2, color)    matrix.drawLine(row1, col1, row2, col2, color)
...
void loop() {
  ...
  for ( int x = 0; x < matrixWidth - 1; x++ ) {
    CLEAR_SCREEN();
    DRAW_LINE(x, 0, matrixWidth - 1 - x, matrixHeight - 1, HIGH);
//    DRAW_LINE(x, random(matrixHeight - 1), matrixWidth - 1 - x, matrixHeight - 1, HIGH);
  ...
  for ( int y = 0; y < matrixHeight - 1; y++ ) {
    CLEAR_SCREEN();
    DRAW_LINE(matrixWidth - 1, y, 0, matrixHeight - 1 - y, HIGH);
//    DRAW_LINE(matrixWidth - 1, y, random(matrixWidth - 1), matrixHeight - 1 - y, HIGH);
    WRITE_SCREEN(); // Send bitmap to display
    delay(wait);
  }
